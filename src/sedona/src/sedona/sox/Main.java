...
{ 
  ...
  public static void usage()
  {
    ...
    println("  disc        Scan the net for activite device");
    println();
  }
  ...
  {               
    ...
    if (cmd.equals("disc"))     return disc(c, args);
  ...
  public static int disc(SoxClient c, String[] args)
      throws Exception
  {
      DaspSocket sock = DaspSocket.open(-1, null, DaspSocket.SESSION_QUEUING);
      println("Send discover msg ...");
      sock.discover(c.port, false);
      println("Send discover msg done, wait ...");
      Thread.sleep(10*1000);
      DiscoveredNode[] discNode = sock.getDiscovered();
      println("NODE: " + discNode[0].toString());
      //for (int i=0; i<discNode.size(); i++)
      //{
      //    println("NODE: " + (DiscoveredNode)(discNode.get(i)).toString());
      //}
      sock.close();
      return 0;
  }
