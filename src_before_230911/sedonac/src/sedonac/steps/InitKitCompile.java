  ...
  extends CompilerStep
{
  ...
  private void validateNames()
  {
    ...
    if (!vendor.equals("Tridium") && !vendor.equals("XunGaoAuto"))
    {
      try
      {
  ...
  void findSourceFiles(ArrayList acc, XElem xml)
  {
    ...
    if (!dir.exists())
      throw err("Unknown source '" + dirName + "'", new Location(xml));
    ...
    {
      if (dir.getName().endsWith(".sedona"))
      {
        try
        {
          SourceFile sf = new SourceFile();
          sf.file = dir.getCanonicalFile();
          sf.testOnly = testOnly;
          acc.add(sf);
        }
        catch (IOException e)
        {
          throw err("Cannot map to canonical filename: " + dir, new Location(xml), e);
        }
      }
    }
  }
